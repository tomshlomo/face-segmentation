...
model = UNetLight(n_channels=train_loader.dataset.num_channels, n_classes=train_loader.dataset.num_classes, k=16)
...
# epoch_bar = tqdm(range(1, n_epochs+1), position=0, desc=' Epochs', leave=True, postfix={'train_loss': 0.0,
#                                                                             'train_acc': 0.0,
#                                                                             'val_loss': 0.0,
#                                                                             'val_acc': 0.0})
...
for epoch in epoch_bar:
    ...
    train_acc = 0.0
    model.train()
    # bar = tqdm(train_loader, position=1, desc=' Training', postfix={"train_loss": 0.0, "train_acc": 0.0})
    for data, target in train_loader:
        ...
        # bar.set_postfix(ordered_dict={"train_loss": batch_loss.item(), "train_acc": batch_acc})
    ...
    model.eval()
    with torch.no_grad():
        # bar = tqdm(val_loader, position=1, desc=' Validating', postfix={"val_loss": 0.0, "val_acc": 0.0})
        for data, target in val_loader:
            ...
            # bar.set_postfix(ordered_dict={"val_loss": batch_loss.item(), "val_acc": batch_acc})
    if epoch % plot_every == 1:
    ...
    # epoch_bar.set_postfix({'train_loss': train_loss,
    #                        'train_acc': train_acc,
    #                        'val_loss': val_loss,
    #                        'val_acc': val_acc})
    epoch_bar.write('epoch: {}  train_loss: {:.3f}  val_loss: {:.3f}  '
          'train acc: {:.3f}  val_acc: {:.3f}'
          .format(epoch, train_loss, val_loss, train_acc, val_acc))

    # save model (if improved on val set)
    if val_loss <= valid_loss_min:
        epoch_bar.write('Validation loss decreased ({:.3f} --> {:.3f}).  Saving model ...'.format(valid_loss_min, val_loss))
